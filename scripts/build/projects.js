...
const projectBuilder = new ProjectQueue((projects, lkg, force) => exec(Path.join(Path.dirname(process.execPath), "npx"), ["tsc", "-b", ...(force ? ["--force"] : []), ...projects], { hidePrompt: true }));
...
const projectCleaner = new ProjectQueue((projects, lkg) => exec(Path.join(Path.dirname(process.execPath), "npx"), ["tsc", "-b", "--clean", ...projects], { hidePrompt: true }));
...
const projectWatcher = new ProjectQueue((projects) => exec(Path.join(Path.dirname(process.execPath), "npx"), ["tsc", "-b", "--watch", ...projects], { hidePrompt: true }));
